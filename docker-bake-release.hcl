...
target "cuda" {
  ...
  tags = [
    "quay.io/vllm/vllm-cuda:${replace(VLLM_VERSION, "+", "_")}", # vllm_version might contain local version specifiers (+) which are not valid tags
  ]
}
...
target "rocm" {
  ...
  tags = [
    "quay.io/vllm/vllm-rocm:${replace(VLLM_VERSION, "+", "_")}", # vllm_version might contain local version specifiers (+) which are not valid tags
  ]
}
